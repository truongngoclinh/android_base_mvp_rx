...
public class MSCameraUtil {
    ...
    public static Uri getOutputMediaFileUri(Context context, File file) {
        return FileProvider.getUriForFile(context, "linhtruong.com.mershop.provider", file);
    }
    ...
    public static File getOutputMediaFile(int type) {
        ...
        File mediaStorageDir = new File(
                ...
                MSConstValue.GALLERY.DIRECTORY_NAME);

        // Create the storage directory if it does not exist
        if (!mediaStorageDir.exists()) {
            if (!mediaStorageDir.mkdirs()) {
                MSLogUtil.e(MSConstValue.GALLERY.DIRECTORY_NAME, "Oops! Failed create "
                        + MSConstValue.GALLERY.DIRECTORY_NAME + " directory");
                return null;
            }
        }
        ...
        File mediaFile;
        if (type == MSConstValue.GALLERY.MEDIA_TYPE_IMAGE) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "IMG_" + timeStamp + "." + MSConstValue.GALLERY.IMAGE_EXTENSION);
        } else {
            return null;
        }
